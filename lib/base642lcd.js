...
Base64Tolcd.prototype.convert = function (base64){
    return new Promise((resolve, reject) => {
        base64ToImageData(base64).then(res => {
            var buffer = tolcd(res);
            resolve(hex2hex(buffer.toString("hex")).join(","));
        }).catch(err => {
            reject("img error!");
        })
    })
...
module.exports = new Base64Tolcd();
